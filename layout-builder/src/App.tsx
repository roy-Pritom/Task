...
//   color: string;
//   width: string; // Track width as a percentage
//   height: string; // Track height as a percentage
// }
...
//     { id: 1, parentId: null, isHorizontal: false, color: randomColor(), width: '100%', height: '100%' },
//   ]);
...
//   const handleResizeStop: ResizeCallback = (e, direction, ref, d, partition) => {
...
const App: React.FC = () => {
  ...
  // Handle resizing of partitions
  const handleResizeStop = (_e: MouseEvent | TouchEvent, _direction: string, ref: HTMLElement, _d: NumberSize, partition: Partition) => {
    const parentWidth = ref.parentElement!.offsetWidth;
    const parentHeight = ref.parentElement!.offsetHeight;
    //  console.log(e,direction,ref,d,partition);
    // Update the partition's size based on resizing
    setPartitions(prev =>
      prev.map(p =>
        p.id === partition.id
